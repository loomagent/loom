...
**实测补充**(共 25 组:3–4 个模型 × 4 个任务形态;**只有形态 d 带副作用工具**,其余只读):混批出现 **1 次**,内容是只读的 calculator 与终止工具同批。这个数字要按它的界限读:1/25 的 95% 区间约为 **0.1%–20%**;而且探针的系统提示与工具描述都**显式要求"终止工具单独调用"**,没有对照组,所以真实频率只会更高——它只是点估计,不是"约 4% 很安全"。副作用一项的证据更弱:**d × 4 个模型 = 0/4 次**,对罕见风险几乎没有排除力。

**因此"整批拒绝"作为默认是协议取舍,不是"已被证明更安全"**:它给调用方一个清楚、可验证的契约——"这一批没有任何工具被执行";而"只拒终止调用"是否会导致副作用重复执行,需要实测而非推断。反过来,整批拒绝也不保证整段任务的副作用恰好一次(执行后写结果失败、跨批重试仍有窗口)。

**删除"代价最多一轮"这一说法**:一次拒绝本身占一轮,之后还要重发普通调用再单独收尾;重复拒绝或原地打转**没有轮次上界**(探针只是被自己 6 轮的上限截断)。

**框架自封拒绝循环**(`Config.MaxConsecutiveRefusals`,默认 3,0 即取默认,刻意无法关闭):按**批次**计连续"未执行任何工具"的批次(混批拒绝与预算拒绝都算,换错误码不算进展),任何一次真实执行即归零;达到上限时**先写完本批各 call 的错误结果**,再返回 `*RefusedBatchesError`(带拒绝次数与最后一次原因)。这一层必要,因为 `MaxSteps = 0` 表示**无限轮**,`MaxToolCalls` 只统计成功的普通工具调用、且标记工具被豁免,两者都不是拒绝循环的可靠边界;调用方的轮次/时间/费用预算仍然照设。

**合规测试的界限**:把构造出的"刚被整批拒绝"对话摆给模型,只能说明"看到这段文案后 6 轮内是否按指示继续"——3/4 个模型照做、grok 反复调用同一工具,只能算**文案有可用迹象**;它不能证明自然发生频率、真实拒绝后的恢复率,也不能证明两种策略的相对代价。而且那次构造的前缀本身自相矛盾(伪造的推理说"我查过了",工具结果却说"本批未执行"),这也是为什么它只是迹象。

顺带一条实测约束:**伪造或省略 thinking 模式的 assistant 轮时,DeepSeek 会以 400 要求回传 `reasoning_content`**(本设计已通过结构化推理载体覆盖该路径;迁移与自造历史都要注意)。
