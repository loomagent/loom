...
**实测补充**(共 25 组:3–4 个模型 × 4 个任务变体,其中 4 个变体带副作用工具):混批只出现 **1 次(≈4%)**,内容是只读的 calculator 与终止工具同批;**没有一次**把带有副作用的工具与收尾捆在一起。把"本批已被整批拒绝"的对话摆给模型,4 个模型里 **3 个**按文案补齐了普通调用、再单独调用终止工具,**grok 卡在反复调用同一个工具**——所以**真正的运行边界是调用方的轮次与工具预算**(`MaxSteps` / `MaxToolCalls`),不能指望拒绝文案让每个模型收敛。据此仍以"整批拒绝"为默认:它更安全、触发罕见,代价最多是一轮。
