...
3. **终局保证**:带标记的工具成功后,工具被物理移除;此后每次请求都不带工具,其 `content` 按构造就是正文;终局不可逆;最终答案只写一次。
...
- **"最终答案只写一次"不能只靠 `ErrTurnClosed`**:封口前存在并发提交与"流式失败后重试"两个窗口,需要原子提交占位,或把保证限定为"调用方串行提交且不重开流式候选"。
