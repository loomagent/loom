...
go run ./examples/structured   # 一次结构化调用：先被拒，再满足契约
```
...
忽略的约束。规范定义为整数的**关键字值**必须写成整数:`minLength: 2.0` 会被拒——
而当它作为**实例值**出现时,`2.0` 就是整数,会被读成整数。三类别跳过(未建模关键字、
关键字写法、未实现构造)都记在 `internal/toolcontract` 里,并带数量断言,改动其中一类
就必须同步更新那份记录。
...
该调用**成功**之后,此后每次请求都不带工具,所以正文不可能与工具调用交错;阶段不可逆,每个 Turn **至多提交一份**最终答案——失败的流只是候选,只有已提交的那份进入历史。调用失败则错误作为工具结果回来,阶段保持开放。名字属于调用方:框架没有阶段枚举,`start_report` 与 `finalize_answer` 是同一机制的两种命名。带标记的工具**不会被工具预算扣住**(否则阶段无法结束),`Config.ToolPhaseEndedPrompt` 可覆盖那句告知模型"工具阶段已结束"的提示。
...
把该 assistant 轮回传进第二次请求、以及一次结构化调用。最后一次按该块**声明的能力**走:
默认 `json_object`——由上面那段拼法承载形状,契约必须在一次重试内被满足;或 `json_schema`
——端点从 `response_format` 拿到 schema,套件断言它接受——**声明的能力若是假的,在这里
就会失败,而不是到生产才失败**。这三条是测试断言的对象;随模型
