...
// The limiter decides on wall-clock time, so every test that waits for a
// cooldown or a window runs in a synctest bubble: the clock is fake, the waits
// are instant, and no goroutine can outlive the test.
...
func TestLimiterRateLimitHalvesWindowAndAppliesSharedCooldown(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		synctest.Sleep(45 * time.Millisecond)
...
func TestLimiterServiceUnavailableUsesSharedHalfOpenCircuitWithoutReducingQuota(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		synctest.Sleep(35 * time.Millisecond)
