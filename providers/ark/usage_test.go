			...
			t.Run(fmt.Sprintf("%s/stream=%v", tc.name, stream), func(t *testing.T) {
				...
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					if stream {
						...
						fmt.Fprint(w, "data: {\"model\":\"ark-future\",\"choices\":[{\"delta\":{\"content\":\"answer\"},\"finish_reason\":\"stop\"}]}\n\n")
						...
						fmt.Fprintf(w, `{"model":"ark-future","choices":[{"message":{"content":"answer"},"finish_reason":"stop"}]%s}`, tc.usage)
					}
				}))
				defer server.Close()
