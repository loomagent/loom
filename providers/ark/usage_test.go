...
func TestUsagePresenceOnWire(t *testing.T) {
	cases := []struct {
		...
		{"zero", `,"usage":{"prompt_tokens":5,"completion_tokens":7,"total_tokens":12,"prompt_tokens_details":{"cached_tokens":2},"completion_tokens_details":{"reasoning_tokens":0}}`, true, 0},
			...
			t.Run(fmt.Sprintf("%s/stream=%v", tc.name, stream), func(t *testing.T) {
				...
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					if stream {
						...
						_, _ = fmt.Fprint(w, "data: {\"model\":\"ark-future\",\"choices\":[{\"delta\":{\"content\":\"answer\"},\"finish_reason\":\"stop\"}]}\n\n")
						...
						_, _ = fmt.Fprintf(w, `{"model":"ark-future","choices":[{"message":{"content":"answer"},"finish_reason":"stop"}]%s}`, tc.usage)
					}
				}))
				defer server.Close()
				m, err := New(Config{APIKey: "test", BaseURL: server.URL, ModelName: "ark-future", Retry: &loom.RetryConfig{Mode: loom.RetryModeDisabled}})
				if err != nil {
					t.Fatal(err)
				}
